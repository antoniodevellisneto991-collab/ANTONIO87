...
(function () {
    ...
    dropZone.addEventListener("click", function (e) {
        if (e.target !== btnSelect && !btnSelect.contains(e.target)) fileInput.click();
    });
    ...
    function uploadFile(file) {
        ...
        xhr.upload.addEventListener("progress", function (e) {
            if (e.lengthComputable) {
                progressBar.style.width = Math.round((e.loaded / e.total) * 70 + 15) + "%";
            }
        });
        ...
        xhr.addEventListener("load", function () {
            if (xhr.status === 200) {
                ...
                setTimeout(function () { openBook(data); }, 200);
            } else {
    ...
    btnNext.addEventListener("click", function () {
        if (bookData && currentPage < bookData.pages.length - 1) renderPage(currentPage + 1);
    });
    ...
    btnFontDown.addEventListener("click", function () {
        if (fontSize > 0.85) { fontSize -= 0.08; readerText.style.fontSize = fontSize + "rem"; }
    });
    btnFontUp.addEventListener("click", function () {
        if (fontSize < 2.0) { fontSize += 0.08; readerText.style.fontSize = fontSize + "rem"; }
    });
