/**
 * Google Cloud Book Reader — Frontend
 * Leitor de TXT com vozes neurais WaveNet/Neural2
 */
(function () {
    "use strict";
    ...
    var selPitch = document.getElementById("sel-pitch");
    ...
    // Theme
    function initTheme() {
        var saved = localStorage.getItem("gcr-theme");
    ...
    function setDark(on) {
        ...
        localStorage.setItem("gcr-theme", on ? "dark" : "light");
    }
    btnTheme.addEventListener("click", function () { setDark(document.documentElement.getAttribute("data-theme") !== "dark"); });
    initTheme();

    // Speed & Pitch sliders
    selSpeed.addEventListener("input", function () { speedLabel.textContent = parseFloat(selSpeed.value).toFixed(1) + "x"; });
    selPitch.addEventListener("input", function () { pitchLabel.textContent = selPitch.value; });
    ...
    function handleFile(file) {
        if (!file.name.toLowerCase().endsWith(".txt")) { alert("Selecione um arquivo .txt"); return; }
    ...
    function uploadFile(file) {
        ...
        xhr.upload.addEventListener("progress", function (e) {
            if (e.lengthComputable) progressBar.style.width = Math.round((e.loaded / e.total) * 70 + 15) + "%";
        });
        xhr.addEventListener("load", function () {
            if (xhr.status === 200) {
                progressBar.style.width = "100%";
                progressText.textContent = "Pronto!";
                setTimeout(function () { openBook(JSON.parse(xhr.responseText)); }, 200);
            } else {
                var err = JSON.parse(xhr.responseText);
                progressText.textContent = "Erro: " + (err.error || "Falha");
    ...
    function openBook(data) {
        ...
        document.title = data.filename + " — Google Cloud Reader";
    ...
    function renderPage(idx) {
        ...
        currentPage = idx;
        readerText.textContent = bookData.pages[idx];
    ...
    btnBack.addEventListener("click", function () {
        stopAudio(); autoMode = false; btnAuto.classList.remove("active");
        bookData = null; screenReader.classList.add("hidden"); screenUpload.classList.remove("hidden");
        resetUpload(); document.title = "Google Cloud Reader";
    });
    ...
    btnAuto.addEventListener("click", function () {
        ...
        if (autoMode && !isPlaying) btnSpeak.click();
    });
    ...
    btnSpeak.addEventListener("click", function () {
        ...
        speakLabel.innerHTML = '<span class="spinner"></span> Gerando...';
        ...
        fetch("/tts", {
            ...
            body: JSON.stringify({
                ...
                pitch: parseFloat(selPitch.value),
            }),
        })
    ...
    function stopAudio() {
        ...
        iconPlay.classList.remove("hidden"); iconStop.classList.add("hidden");
        speakLabel.textContent = "Ouvir Página";
        audioBarWrap.classList.add("hidden"); audioFill.style.width = "0%"; audioTime.textContent = "0:00";
    }
})();
